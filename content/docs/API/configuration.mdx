...
| [`use_active_record_presence`](#use_active_record_presence)                             | `Boolean`                                                 | `true`  |
...
For example, `validates :role, presence: true` will infer `role` as non-null regardless of whether the database has a non-null constraint on the `role` column.
