...
- Type: `({ reason: String } -> String) | String`

Whether to exclude this resource from the schema. By default, if not defined on the resource, the value is `"Abstract."` if `JSONAPI::Resource._abstract` is `true`, `nil` otherwise.
...
Useful for when you want to generate the schema dynamically, e.g.

```ruby
class Schema < Anchor::Schema
  serializable_resources = JSONAPI::Resource.descendants
    .filter { |klass| klass.singleton_methods.include?(:anchor_schema_name) }
    .reject(&:anchor_exclude_from_schema?)

  serializable_resources.each { |klass| resource(klass) }
end
```
